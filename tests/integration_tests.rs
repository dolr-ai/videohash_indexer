...
use std::sync::Arc;
use videohash_indexer::{create_shared_index, delete_hash, search, SearchRequest, VideoHashIndex};
...
async fn test_search_find_similar_hash() {
    ...
    assert!(
        response["match_details"]["similarity_percentage"]
            .as_f64()
            .unwrap()
            > 90.0
    );
